...
class CommunicationTrackingMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.verification_summary: dict[str, object] = {
            "enabled": True,
            "passed": False,
    ...
    def _run_preflight_verification(self, grid: Grid, timeout: float) -> dict[str, object]:
        ...
        if isinstance(expected_clients, dict):
            expected_partition_ids = sorted(expected_clients.keys())
            ...
            for partition_id, expected in expected_clients.items():
                observed = observed_clients.get(partition_id)
                if observed is None:
                    continue
...
def build_communication_summary(
    ...
    artifact = {
        "verification": verification_summary or {
            "enabled": False,
            "passed": False,
            "errors": ["verification_summary_missing"],
        },
        "totals": {
