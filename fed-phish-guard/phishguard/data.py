...
def get_class_weights(
    labels: list[int], device: Optional[torch.device] = None
) -> torch.Tensor:
...
def _extract_partition_id(
    hf_ds: HFDataset, fallback_partition_id: int | str
) -> int | str:
    """Return the natural client id when the partition contains exactly one."""
...
def _dataset_fingerprint(hf_ds: HFDataset) -> str:
    ...
    client_ids = hf_ds[PARTITION_COL] if PARTITION_COL in hf_ds.column_names else None
    ...
    for idx, (url, label) in enumerate(zip(urls, labels)):
        ...
        if client_ids is not None:
            hasher.update(b"|")
            hasher.update(str(client_ids[idx]).encode("utf-8"))
        hasher.update(b"\n")
...
def _make_partitioner(partitioner_name: str, num_partitions: int):
    ...
    raise ValueError(
        f"Unsupported partitioner '{partitioner_name}'. " "Use 'iid' or 'natural'."
    )
...
def load_sim_data(
    ...
    if partitioner_name.strip().lower() == "natural":
        ...
        if len(client_ids) != 1:
            raise RuntimeError(
                "NaturalIdPartitioner returned a mixed-client partition."
            )
